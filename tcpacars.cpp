...
tcpacars::tcpacars(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::tcpacars)
...

{
    ui->setupUi(this);
    socketaacars = NULL;
    socketaacars = new QTcpSocket(this);

    connect (socketaacars, &QTcpSocket::readyRead, [&](){

        QByteArray buffer;
        buffer.resize( socketaacars->bytesAvailable() );
        socketaacars->read( buffer.data(), buffer.size() );
        QByteArray ba_as_hex_string = buffer.toHex();
        ui->plainTextEdit->setReadOnly( true );

        if ((buffer[5] == 80) && (buffer[8] >= 18)){

            int PARAM_CONF = ui-> frequence_7 -> value();
        ...
        if (((buffer[8] == 47) || (buffer[8] == 44) || (buffer[8] == 40)) && ( buffer[9] == 1)){
            ...
            int PWR_SET_REQ = ui -> puissance_7 -> value();
    ...
    connect (hd, &QTcpSocket::readyRead, [&](){

        QByteArray buffer;
        buffer.resize( hd->bytesAvailable() );
        hd->read( buffer.data(), buffer.size() );
        ui->plainTextEdit->setReadOnly( true );
        QByteArray ba_as_hex_string = buffer.toHex();
...
void tcpacars::on_actionPARAM_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/

    QString ip = ui->lineEditip_14->text();
    quint16 port = ui->spinBox_19->value();

    socketaacars->connectToHost(QHostAddress(ip),port);
    ...
    int c = 0;
    int msb[2] , lsb[2] ;
    int monnombre = ui->frequence_7->value();
    int mynumber = 0;
    int p =1 , e = 1 , o = 1;
   ...
   for(int i = 1; i<5 ; i++){
    ...
    uint8_t var4;
    if (ui -> prekey_7 -> value() >= 0 && ui -> prekey_7 -> value() <= 85 ){
        var4 = ui -> prekey_7 -> value();
    }
    else{
    ...
    if  ( ui -> frequence_7 -> value() >= 118000  &  ui -> frequence_7 -> value() <= 136000 )
            {
                tsPid20 Pid20;
               ...
               <<(uint8_t)Pid20.freq_lsb<<(uint8_t)Pid20.freq_msb<<(uint8_t)Pid20.mod<<(uint8_t)Pid20.prekey<<(uint16_t)Pid20.crc;
                ...
                socketaacars->write(buffer);
                int PARAM_REQ = ui-> frequence_7 -> value();
...
void tcpacars::on_actionPARAM_REQ_2_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/

    QString ip = ui->lineEditip_13->text();
    quint16 port = ui->spinBox_20->value();

    hd->connectToHost(QHostAddress(ip),port);
...
void tcpacars::on_actionPWR_SET_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpacars::on_actionMODE_SET_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpacars::on_actionMODE_SET_REQ_2_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpacars::on_actionPURGE_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
  ...
  s << (uint8_t)Pid25.synchro [0]<< (uint8_t)Pid25.synchro [1]<<(uint8_t)Pid25.synchro [2]<<(uint8_t)Pid25.synchro [3]  << (uint8_t)Pid25.protocoleCode << (uint8_t)Pid25.pid << (uint8_t)Pid25.lengthMsb <<(uint8_t)Pid25.lengthLsb<<(uint16_t)Pid25.crc;
...
void tcpacars::on_actionUNIT_DATA_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
   ...
   // uint8_t varh = ui-> spinBox -> value() + 27;
    ...
    uint8_t var6 = ((ui-> spinBox_21 -> value())) +28;
    unsigned char str[] ={
    ...
    unsigned char str2[ui-> spinBox_21->value()];
   ...
   unsigned char myData1[ui-> spinBox_21->value()+32];
   ...
   socketaacars->write(buffer2);
 //  tcpsocket111->writeDatagram(buffer2,QHostAddress::LocalHost, 1111);
   counter++;
