...
tcpvdl2::tcpvdl2(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::tcpvdl2)
{
    ui->setupUi(this);
    ...
    socketfdR = new QTcpSocket(this);
    socketsqp = new QUdpSocket(this);
    socketd = new QUdpSocket(this);

    socketfdR->setSocketOption(QAbstractSocket::ReceiveBufferSizeSocketOption, 64 * 1024 * 1024);

    connect(timer, &QTimer::timeout, this, &tcpvdl2::start_timer);

    connect (socketfdR, &QTcpSocket::readyRead, [&](){

        QByteArray buffer;
        buffer.resize( socketfdR->bytesAvailable() );
        socketfdR->read( buffer.data(), buffer.size() );
        ...
        if(buffer[5] == 83){
         ...
         if( buffer[5] == 81 ){
              ...
              ui->  plainTextEdit_3->appendPlainText(ba_as_hex_string);
              socketd->writeDatagram(buffer,QHostAddress::LocalHost,1964);
    ...
    connect (socketfd, &QTcpSocket::readyRead, [&](){
           QByteArray buffer;
           buffer.resize( socketfd->bytesAvailable() );
           socketfd->read( buffer.data(), buffer.size() );
           ui->plainTextEdit->setReadOnly( true );
           QByteArray ba_as_hex_string = buffer.toHex();
...
void tcpvdl2::on_actionPARAM_req_triggered()
{
    ...
    socketfd->connectToHost(QHostAddress(ip),port);
...
void tcpvdl2::on_actionPWR_SET_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpvdl2::on_actionMODE_SET_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpvdl2::on_actionPURGRE_REQ_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpvdl2::on_actionPARAM_REQ_triggered()
{
        ...
        socketfdR->connectToHost(QHostAddress(ip),port);
...
void tcpvdl2::on_actionMODE_SET_REQ_2_triggered()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpvdl2::on_pushButton_clicked()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
    ...
    quint16 port2 = ui->spinBox_10->value();

    socketfdR->connectToHost(QHostAddress(ip1),port2);
    /*-----------------------------------------------------------------------------------*/
    // Algo de traitement de la fréquence
    //Il faut créer une liste qui prend la taille de la valeur de fréquence désiré
    //mettre le nombre saisie dans le tableu sous forme de chiffre
...
void tcpvdl2::on_UINT_DATA_REQ_clicked()
{
    /*------------------------Se connecter à la radio---------------------------------------------------*/
...
void tcpvdl2::on_pushButton_3_clicked()
{
    timer->start(1000);
...
void tcpvdl2::on_pushButton_2_clicked()
{
    timer->stop();

}

void tcpvdl2::on_pushButton_4_clicked()
{
    form3.show();
...
void tcpvdl2::on_version_clicked()
{
    version.show();

}

void tcpvdl2::start_timer(){
    /*------------------------Se connecter à la radio---------------------------------------------------*/
